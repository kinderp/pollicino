...
EXPERT_NAMES = ("adaptive-o3", "neural-prior-256", "neural-prior-1024")
...
def download_and_unpack(p4, url: str, path: Path, expected: dict[str, tuple[str, int]]) -> tuple[bytes, dict[str, bytes]]:
...
def main() -> None:
    ...
    p4 = load_module(ROOT / "experiments/pilot-004/run.py", "pilot004_for_p6")
    p5 = load_module(ROOT / "experiments/pilot-005/run.py", "pilot005_for_p6")
    ...
    expert_fps = (adaptive_fp, prior256_fp, prior1024_fp)
    ...
    def encode_only(data: bytes, window: int) -> dict:
        provider = gate_factory(window)()
        started = time.perf_counter()
        blob = encode_shared(data, provider, gate_fp(window), precision_bits=PRECISION)
        ...
        return {
            ...
            "choice_fractions": provider.choice_fractions(),
        }

    def roundtrip_gate(data: bytes, window: int) -> dict:
        ...
        restored = decode_pol(
            ...
            expected_model_fingerprint=gate_fp(window),
        )
    ...
    # Canterbury + artificial are development data: the gate window is selected here.
    ...
    for window in DEV_WINDOWS:
        ...
        for name in dev_names:
            ...
            result = encode_only(data, window)
            payload_values.append(result["payload_bpb"])
            dev_rows.append({
                "window": window,
                "file": name,
        ...
        print("DEV-MEAN", window, mean(payload_values), flush=True)
    window_means = {
    ...
    # Calgary is untouched holdout for the selected gate.
    calgary_rows = []
    for name, data in cal.items():
        sample = data[:SLICE]
        gate = roundtrip_gate(sample, selected_window)
        adaptive = roundtrip_adaptive(sample)
        bases = baseline_bpb(sample)
        eval_n = min(65536, len(data))
        adaptive_long = p5.adaptive_bpb(data, eval_n, ADAPTIVE_CFG)
        ...
        calgary_rows.append({
            ...
            "long_adaptive_o3_bpb": adaptive_long,
    ...
    for name, category, data in [
        ...
        gate = roundtrip_gate(data, selected_window)
    ...
    weighted_gate = mean(r["gate_payload_bpb"] for r in calgary_rows)
    weighted_adaptive = mean(r["adaptive_payload_bpb"] for r in calgary_rows)
    weighted_zlib = mean(r["zlib_bpb"] for r in calgary_rows)
    neural_choice = mean(
        r["gate_choice_fractions"].get("neural-prior-256", 0.0)
        + r["gate_choice_fractions"].get("neural-prior-1024", 0.0)
    ...
    results = {
        ...
        "aggregate_holdout_2k": {
            "mean_gate_payload_bpb": weighted_gate,
            "mean_adaptive_o3_payload_bpb": weighted_adaptive,
            "mean_zlib_bpb": weighted_zlib,
            ...
            "mean_neural_prior_choice_fraction": neural_choice,
        },
        ...
        "limits": [
            ...
            "The gate requires the shared neural checkpoint even when the adaptive expert is selected most of the time.",
