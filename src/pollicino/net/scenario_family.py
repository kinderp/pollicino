...
from .fair_scheduling import BearerSchedulingPolicy
...
class SyntheticScenarioFamilyConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_ttl_seconds > 0xFFFFFFFF:
            raise ValueError("max_ttl_seconds must fit in the PND1 unsigned 32-bit field")
...
def _make_windows(
    ...
    *,
    ...
    for index in range(config.windows_per_scenario):
        ...
        # Deliberately independent draw: duration never drives capacity here.
        budget = rng.randint(
...
def _descriptor_key(
    ...
    """Stable bounded PND1 rendezvous key for arbitrarily long family IDs."""

    return hashlib.sha256(
        f"family:{family_id}:scenario:{scenario_index}:bundle:{bundle_index}".encode(
            "utf-8"
        )
    ).digest()[:16]
    ...
    *,
    ...
    for index in range(config.bundle_count):
        ...
        descriptor = DiscoveryDescriptor(
            object_class=1,
            rendezvous_key=_descriptor_key(
                config.family_id,
                scenario_index,
                index,
            ),
...
def generate_synthetic_scenario_family(
    ...
    *,
    ...
    seed produces stable bundle identities and contact metadata. The generator
    never derives logical byte budget from contact duration and never labels
    generated values as physical measurements.
    """
    ...
    if not all(isinstance(item, SyntheticBearerTemplate) for item in bearer_templates):
        raise TypeError("bearer_templates must contain SyntheticBearerTemplate values")
    if len({item.profile.bearer_id for item in bearer_templates}) != len(
        bearer_templates
    ):
    ...
    for scenario_index in range(config.scenario_count):
        ...
        if len(strategy_ids) != len(set(strategy_ids)):
            raise ValueError("strategy_factory must return unique strategy IDs")
        ...
        priority_counts = tuple(
            (
                priority.name.lower(),
                sum(item.priority is priority for item in bundles),
            )
        ...
        bearer_counts = tuple(
            (
                ...
                sum(
                    window.bearer_id == template.profile.bearer_id
                    for window in windows
                ),
            )
            for template in sorted(
                bearer_templates,
                key=lambda item: item.profile.bearer_id,
            )
        )
        summaries.append(
