...
if nn is not None:
    class CausalSelfAttention(nn.Module):
        ...
        def __call__(self,x):
            b,t,c=x.shape; qkv=self.qkv(x); q,k,v=mx.split(qkv,3,axis=-1)
            def split(z): return mx.transpose(mx.reshape(z,(b,t,self.n_heads,self.head_dim)),(0,2,1,3))
            q,k,v=split(q),split(k),split(v); scores=(q @ mx.swapaxes(k,-1,-2))/math.sqrt(self.head_dim)
            future=mx.arange(t)[None,:] > mx.arange(t)[:,None]; scores=mx.where(future[None,None,:,:],-1e9,scores)
            weights=mx.softmax(scores,axis=-1); out=weights @ v; out=mx.reshape(mx.transpose(out,(0,2,1,3)),(b,t,c)); return self.proj(out)
